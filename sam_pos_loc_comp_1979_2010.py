...
f1upper.set_ylim( [18 , 42] )

f1upper.legend( loc=2 , ncol=3, prop={'size':12},bbox_to_anchor=(0.1, 1), handlelength=1.25)
f1upper.set_xticklabels([])
f1upper.set_xlabel('')
f1upper.text( datetime(1873,01,01), 39, 'a)')
...
f2upper.legend( ncol=1, prop={'size':12}, bbox_to_anchor=(1.5, 1), handlelength=1)
f2upper.text(-0.25,1.65,'a)')
...
f1lower.set_xticklabels([])
f1lower.set_xlabel('')
f1lower.text( datetime(1873,01,01), 8.85, 'b)')
...
for (i, name) in enumerate( rean ):
    ...
    if ( dfr.index.year.min() > tys ):
        ...
        # Plot the annual mean time-series 
        plt.figure(1)
        f1bottom = plt.subplot(313)
        f1bottom.xaxis.grid(color=[0.6,0.6,0.6])
        f1bottom.yaxis.grid(color=[0.6,0.6,0.6])
        f1bottom.set_axisbelow(True)
        plot_loc_ts( dfr , tys , tye , rlc[i] )
 
        plt.figure(2)
        f2bottom = plt.subplot( 325 )
        for ( k, nm ) in enumerate( seas ):
            names = 'dfr.seasons.' + nm
...
f1bottom.set_xticklabels( [ str(jj.year) for jj in xtics ],rotation=30)
f1bottom.set_xticks(xtics)
f1bottom.set_xlim( [datetime(1871,1,1) , datetime(2012,12,31) ] )

plt.figure(1).subplots_adjust(hspace=0.1)
f1bottom.yaxis.set_major_locator( mticker.MaxNLocator(7,prune='upper') )
f1lower.yaxis.set_major_locator( mticker.MaxNLocator(7,prune='upper') )

f1bottom.text( datetime(1873,01,01), -47.25, 'c)')
...
f2lower.yaxis.set_major_locator( mticker.MaxNLocator(6,prune='both') )
f2bottom.yaxis.set_major_locator( mticker.MaxNLocator(6,prune='upper') )
plt.figure(2).subplots_adjust(hspace=0.1)
plt.figure(2).subplots_adjust(left=0.2)
plt.figure(2).subplots_adjust(right=0.9)

f2bottom.text( -0.25,0.75,'c)')
