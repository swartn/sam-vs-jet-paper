...
def rean_proc(dfr, axts='', axtrend='', tys=0, tye=0):
    ...
    for (i, name) in enumerate( rean ):      
        ...
        if ( axts ):  
            axts.plot( dfr.resample('A').index, dfr[i+1].resample('A'), color=rlc[ i ], linewidth=3, alpha=1, label=name)
...
f2a = plt.subplot(421)
rean_proc(press, axtrend=f2a, tys=tys, tye=tye)   
...
mod_proc(modpress, f2a, tys=tys, tye=tye)
...
plt.figure(2)
f2b = plt.subplot(423)
rean_proc(maxspd, axtrend=f2b, tys=tys, tye=tye)   
...
# now do the monthly trends
mod_proc(modmaxspd, f2b, tys=tys, tye=tye)
...
plt.figure(2)
f2c = plt.subplot(425)
rean_proc(locmax, axtrend=f2c, tys=tys, tye=tye)   
...
# now do the monthly trends
mod_proc(modlocmax, f2c, tys=tys, tye=tye)
...
plt.figure(2)
f2d = plt.subplot(427)
rean_proc(width, axtrend=f2d, tys=tys, tye=tye)   
...
# now do the monthly trends
mod_proc(modwidth, f2d, tys=tys, tye=tye)
...
plt.figure(2).subplots_adjust(hspace=0.05, right=0.7)
f2a.legend( ncol=1, prop={'size':12}, bbox_to_anchor=(1.6, 1.05), handlelength=0.01, handletextpad=1, borderpad=1 )
