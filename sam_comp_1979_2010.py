...
locmax = pd.read_csv('rean_uloc.txt',names=['date','ind','rno','loc']) # read in
...
for (i, name) in enumerate( rean ):
    ...
    if ( dfr.index.year.min() > tys ):
       ...
       # plot the time-series
        ...
        plt.axis([-0.5, lensea -0.5 , -0.55, 2])
        plt.plot([-1, 5],[0, 0], 'k--')
...
plt.savefig('sam_jet_ts_1979_2010.pdf',format='pdf',dpi=300,bbox_inches='tight')
...
[ gs2[m,n].text(-0.6,0.4, seas_label[m][n].upper() ) for m in range(3) for n in range(2)  ]
plt.draw()
plt.savefig('sam_v_jet_1979_2010.pdf',format='pdf',dpi=300,bbox_inches='tight')
