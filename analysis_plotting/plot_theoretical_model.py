...
def sam_model(strength, position, width):
    ...
    and width on the latitude grid given in y. Then it uses the generated zonal 
    windspeed and indefinitely integrates to find the pressure via the inverted 
    geostrophic relationship, and finally calculates the sam index, which is returned
    """
...
def splot(gs, lat, pres, u, sam, param, param0, col='k', cp=0):
    """ Plots a zonal mean jet profile and SAM vs jet properties. 
    """
    # jet defaults (defined here and below - but should be defined only once)
    strength0 = 7 # m/s
    position0 = -48 # degrees
    width0    =  np.sqrt(32) # degrees latitude
    gs[0,cp].plot(lat, pres, '-', color=col)   
    gs[0,cp].axis( [-70, -30, 980, 1015])
    gs[1,cp].plot(lat, u, '-', color=col)
    gs[1,cp].axis( [-70, -30, 0, 10])
    ...
    gs[2,cp].plot([-10,10], [0,0],'--k', linewidth=0.5, color=[0.5, 0.5, 0.5], 
                  zorder=1)
    gs[2,cp].plot([0,0], [-5,5],'--k',linewidth=0.5, color=[0.5, 0.5, 0.5], 
                  zorder=2)
...
def plot_theoretical_model():
    """ Main routine to make the plot
    """
    
    # Define some default parameters
    lat = np.linspace( -70 , -20 , 100 ); # latitudes
    dy = lat[1] - lat[0] # change in latitude
    deg2rad = np.pi / 180 # conversion to radians
    rho = 1.2 ; # density of air 
    ...
    # jet defaults (defined here and above - but should be defined only once)
    strength0 = 7 # m/s
    position0 = -48 # degrees
    width0    =  np.sqrt(32) # degrees latitude
    ...
    for i,str in enumerate(np.linspace( strength0*0.75, strength0*1.25, 8)):
        pres, sam, u, Strength = sam_model(strength=str, position=position0, 
                                           width=width0 )
        splot(gs, lat, pres, u, sam, str, strength0,col=col[i], cp=0)
    ...
    for i,pos in enumerate( np.linspace( position0+4, position0-4, 8 ) ):
        pres, sam, u, Strength = sam_model(strength=strength0, position=pos, 
                                           width=width0)
        splot(gs, lat, pres, u, sam, pos, position0,col=col[i], cp=1)
    ...
    for i,wid in enumerate( np.linspace( width0*0.75, width0*1.25, 8 ) ):
        pres, sam, u, Strength = sam_model(strength=strength0, position=position0, 
                                           width=wid)
        splot(gs, lat, pres, u, sam, wid, width0, col=col[i], cp=2)
