...
f1a = plt.subplot( 321 )
plt.figure(1).subplots_adjust(hspace=0.05,right=1.25)
...
f2e = plt.subplot(322, sharey=f2a)
...
#f1a.legend( ncol=3, prop={'size':12}, bbox_to_anchor=(0.775, 0.975), handlelength=1.25, handletextpad=0.075, frameon=False)
f1a.legend( ncol=1, prop={'size':12}, bbox_to_anchor=(1.25, 1.05), handlelength=1.25, handletextpad=0.075, frameon=False )
...
f2f = plt.subplot(324, sharey=f2b)
...
f2g = plt.subplot(326, sharey=f2c)
...
f1ax = [ f1a, f1b, f1c]
panlab = ['a)', 'b)', 'c)', 'e)', 'f)' ,'g)']
yaxlab1 = ['SAM Index (hPa)' , 'Umax (m/s)','Position ($^{\circ}$S)']
...
for i, ax in enumerate( f1ax ):
    ...
    ax.text( datetime(1882,1,1), max( ylim ) -0.135*yrange, panlab[i])
...
# FIGURE 2: Trends
# defines some lists of labels.
f2ax = [ f2a, f2b, f2c, f2e, f2f, f2g]
yaxlab = ['SAM trend \n(hPa/dec)', 'Umax trend \n(ms$^{-1}$/dec)', 'Position trend \n($^{\circ}$ lat./dec)']

f2c.set_xticklabels(  [ s.upper() for s in seas]  )
f2g.set_xticklabels(  [ s.upper() for s in seas]  )
...
for i, ax in enumerate( f2ax ):
    ...
    if (ax != f2c) & (ax != f2g): # only keep xlabels for the bottom panels
        ax.set_xticklabels([])
        ax.set_xlabel('')
...
f2e.legend( ncol=1, prop={'size':12}, bbox_to_anchor=(1.6, 1.05), handlelength=0.01, handletextpad=1, borderpad=1, frameon=False )
...
# save some pdfs
plt.figure(1).savefig('sam_pos_str_width_ts_v4_crdtalk.pdf',format='pdf',dpi=300,bbox_inches='tight')
plt.figure(2).savefig('sam_pos_str_width_trends_v4_crdtalk.pdf',format='pdf',dpi=300,bbox_inches='tight')
