...
mf.set_size_inches(8,10, forward=True)

strength0 = 7 # m/s
position0 = -52 # degrees
...
def splot( lat, pres, u, sam, param, param0, col='k', cp=0):
    gs[cp, 0].plot( lat , u , '-', color=col ) ; 
    gs[cp, 0].axis( [-70, -30, 0, 10])
    #gs[cp, 0].yaxis.set_major_locator( mticker.MaxNLocator(6, prune='both'))
    xtics=np.arange(-70,-30,10)
    gs[cp,0].set_xticks(xtics )


    refpres, refsam, refu, refumax = sam_model(strength0, position0, width0)
    gs[cp,1].plot( param-param0, sam - refsam,'o', color=col,zorder=5 )
    gs[cp,1].plot( [-10,10] , [0,0] ,'--k', linewidth=0.5, color=[0.5, 0.5, 0.5] ,zorder=1)
    gs[cp,1].plot( [0,0] , [-5,5] ,'--k',linewidth=0.5, color=[0.5, 0.5, 0.5], zorder=2)
    gs[cp,1].yaxis.tick_right()
    gs[cp,1].yaxis.set_label_position("right")
    gs[cp,1].set_ylim([-5, 5])
    gs[cp,1].set_ylabel('$\Delta$ SAM (hPa)')
         
    if cp > 1:
        gs[cp,0].set_xlabel('Latitude')
    else:
	gs[cp, 0].set_xticklabels([])
        gs[1,0].set_ylabel('U (m/s)')

col = cmap=brewer2mpl.get_map('RdBu', 'diverging', 8,reverse=True).mpl_colors
# do a trend in strength
#pres, sam, u, umax = sam_model(strength0, position0, width0)
#splot( lat, pres, u, sam, umax, cp=0)
...
gs[0,0].text(-68, 8.5, 'Umax')   
gs[0,1].set_xlabel('$\Delta$ Umax (m/s)')
gs[0,1].set_xlim([-5, 5])

# do a trend in position
#pres, sam, u, umax = sam_model(strength0, position0, width0)
#splot( lat, pres, u, sam, umax, cp=1)

for i,pos in enumerate( np.linspace( position0+8, position0-2, 8 ) ):
    pres, sam, u, umax = sam_model(strength=strength0, position=pos, width=width0 )
    splot( lat, pres, u, sam, pos, position0,col=col[i], cp=1)

gs[1,0].text(-68, 8.5,'Position')    
gs[1,1].set_xlabel('$\Delta$ pos. ($^{\circ}$ lat.)')
gs[1,1].set_xlim([-5, 10])
# do a trend in width
#pres, sam, u, umax = sam_model(strength0, position0, width0)
#splot( lat, pres, u, sam, umax, cp=2)
...
gs[2,0].text(-68, 8.5,'Width')     
gs[2,1].set_xlabel('$\Delta$ width ($^{\circ}$ lat.)')
gs[2,1].set_xlim([-5, 5])    
