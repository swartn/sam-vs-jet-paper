...
def gen_sam_umax( lat_c0, times, dsig=dsig, dstr=dstr,dlatc=dlatc ):
    ...
    return times, sam, umax, pres, u, latc    
    
def sam_umax_trend(lat_c0, dsig = dsig, dstr = dstr ):
    # do trends in strength and position and plot
    times = np.arange(30)
    times, sam, umax, pres, u, latc = gen_sam_umax(lat_c0, times, dsig = dsig, dstr = dstr )
    sam_trend = trends( np.squeeze(sam), times )
    pos_trend = trends( np.squeeze(latc), times )
    umax_trend = trends( np.squeeze(umax), times )
    
    return sam_trend / abs(pos_trend), umax_trend

# Get initial values and plot    
times, sam, umax, pres, u0, latc = gen_sam_umax(lat_c0=locmax.mean().mean(), times=[0], dsig = dsig, dstr = dstr )
gs[0].plot( lat_p, u0 , 'b')
times, sam, umax, pres, u0, latc = gen_sam_umax(lat_c0=locmax.mean().mean(), times=[29], dsig = dsig, dstr = dstr )
gs[0].plot( lat_p, u0 , 'b--')
...
gs[1].plot( rean_pos, rean_sam_trends, 'kx')
gs[1].plot( mod_pos, mod_sam_trends , 'ro')
gs[1].set_ylabel(r'SAM trend (hPa/dec)')
gs[1].set_xlabel(r'Initial position')
