...
uas_slope_c5 = h5f['uas/1979_2011/c5_uas_trend_1979_2011'][:]*120
...
psl_slope_rean = h5f['psl/1979_2011/rean_psl_trend_1979_2011'][:]*120
uas_slope_rean = h5f['uas/1979_2011/rean_uas_trend_1979_2011'][:]*120
uas_slope_rean_88 = h5f['uas/1988_2011/rean_uas_trend_1988_2011'][:]*120
uflx_slope_rean_88 = h5f['uflx/1988_2011/rean_uflx_trend_1988_2011'][:]*120
h5f.close()
...
uflx_slope_ccmp = uflx_slope_ccmp*1.2*1.4e-3
# load in the Marshall SAM data
df = pd.read_csv('/HOME/ncs/data/marshall_sam/marshall_sam.csv', 
		  index_col=0, parse_dates=True)

df = pt.time_lim(df, pd.datetime(1979,1,1), pd.datetime(2011,12,31))
...
fig.subplots_adjust(right=0.5, hspace=0.1)
...
axm.plot(dims['lat'], uas_slope_c5.mean(axis=(0,2)), 'r-', linewidth=3)

axb.plot(dims['lat'], uas_slope_c5_88.mean(axis=(0,2)), 'r-', linewidth=3)
axb.plot(dims['lat'], slope_ccmp.mean(axis=1), 'k-.', linewidth=3)
...
for i,r in enumerate(rean):
    ...
    axm.plot(dims['lat'], uas_slope_rean[:,:,i].mean(axis=1), linestyle='-'
	     , color=rlc[i], linewidth=2) 
    
    axb.plot(dims['lat'], uas_slope_rean_88[:,:,i].mean(axis=1), linestyle='-'
	     , color=rlc[i], linewidth=2) 
